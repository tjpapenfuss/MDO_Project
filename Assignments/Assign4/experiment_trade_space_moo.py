...
imported_df = pd.read_csv("experiment_output_moo.csv")
...
NPV_array = imported_df['NPV'].values
Total_CO2_Gen = imported_df['tot_co2_gen'].values
CAPEX = imported_df['capex'].values
# utility = imported_df['utility'].values
...
plt.scatter(NPV_array, CAPEX, color="lightblue")
# plt.scatter(NPV_array, utility, color="lightblue")
#plt.show()
...
X_values, Y_values = pareto_frontier(NPV_array, CAPEX, maxX = True, maxY = False)
# X_values, Y_values = pareto_frontier(NPV_array, utility, maxX = True, maxY = False)

#print(X_values, Y_values) # Print out to pareto frontier values.

ax = plt.gca()
ax.set_xlim([0, 25000]) # Set the x-axis (NPV) limit to 0-25000
...
plt.savefig('moo_figure.png') # Save the figure to a file

unique_NPV = imported_df["NPV"].nunique()
print(unique_NPV)