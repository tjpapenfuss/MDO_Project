...
imported_df = pd.read_csv("final_df.csv")


# Use the below line of code to just plot all points in a scatter plot
imported_df.plot(x="NPV", y="mtot", kind="scatter", color="lightblue")
...
#     if(row['K'] == 'K3'):
#         plt.scatter(row['NPV'], row['mtot'], color="orange")
#     else:
#         plt.scatter(row['NPV'], row['mtot'], color="lightblue")
...
X_values, Y_values = pareto_frontier(imported_df["NPV"], imported_df["mtot"], maxX = False, maxY = True)
...
plt.plot(imported_df["NPV"][0], imported_df["mtot"][0], marker='s', markersize=8, color="purple")
