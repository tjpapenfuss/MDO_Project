...
def main_effect(dict,var):
    ...
    # Create a scoring matrix for each unique factor in the current list
    factor_score_matrix_capex = np.array([["Factor","Factor Level","Main Effect"]], dtype=object)
    ...
    for key in key_list:
        ...
        for current_factor in factor_list:
            ...
            temp_array = np.array([[str(key), str(current_factor), str(main_effect)]], dtype=object)
            factor_score_matrix_capex = np.vstack((factor_score_matrix_capex, temp_array))

        index += 1
            
    return factor_score_matrix_npv, factor_score_matrix_capex
